...
#ifndef __TBB__x86_rtrw_mutex_impl_H
#define __TBB__x86_rtrw_mutex_impl_H
...
enum RTtype {
    RTnot_in_mutex,
    RTtransacting_reader,
    RTtransacting_writer,
    RTreal_reader,
    RTreal_writer
};
...
class x86_rtrw_mutex: private spin_rw_mutex {
#if __TBB_USE_X86_RTRW_MUTEX || __TBB_GCC_VERSION < 40000
...
private:
#endif
    friend class interface7::internal::padded_mutex<x86_rtrw_mutex,true>;
...
private:
    ...
    void __TBB_EXPORTED_METHOD internal_acquire_writer(x86_rtrw_mutex::scoped_lock&, bool only_speculate=false);

    //! Internal acquire read lock.
    // only_speculate == true if we're doing a try_lock, else false.
    void __TBB_EXPORTED_METHOD internal_acquire_reader(x86_rtrw_mutex::scoped_lock&, bool only_speculate=false);

    //! Internal upgrade reader to become a writer.
    bool __TBB_EXPORTED_METHOD internal_upgrade( x86_rtrw_mutex::scoped_lock& );

    //! Out of line code for downgrading a writer to a reader.
    bool __TBB_EXPORTED_METHOD internal_downgrade( x86_rtrw_mutex::scoped_lock& );

    //! Internal try_acquire write lock.
    bool __TBB_EXPORTED_METHOD internal_try_acquire_writer( x86_rtrw_mutex::scoped_lock& );

    //! Internal release lock.
    void __TBB_EXPORTED_METHOD internal_release( x86_rtrw_mutex::scoped_lock& );

    static x86_rtrw_mutex* internal_get_mutex( const spin_rw_mutex::scoped_lock& lock )
    {
        return static_cast<x86_rtrw_mutex*>( lock.mutex );
    }
...
private:
    ...
    class scoped_lock : tbb::internal::no_copy {
        friend class x86_rtrw_mutex;
        spin_rw_mutex::scoped_lock my_scoped_lock;

        RTtype transaction_state;

    public:
        //! Construct lock that has not acquired a mutex.
        /** Equivalent to zero-initialization of *this. */
        scoped_lock() : my_scoped_lock(), transaction_state(RTnot_in_mutex) {
        }

        //! Acquire lock on given mutex.
        scoped_lock( x86_rtrw_mutex& m, bool write = true ) : my_scoped_lock(),
            transaction_state(RTnot_in_mutex) {
            acquire(m, write);
        }

        //! Release lock (if lock is held).
        ~scoped_lock() {
            if(transaction_state != RTnot_in_mutex) release();
        }

        //! Acquire lock on given mutex.
        void acquire( x86_rtrw_mutex& m, bool write = true ) {
        ...
        //! Release lock
        void release() {
            x86_rtrw_mutex* mutex = x86_rtrw_mutex::internal_get_mutex(my_scoped_lock);
            __TBB_ASSERT( mutex, "lock is not acquired" );
            __TBB_ASSERT( transaction_state!=RTnot_in_mutex, "lock is not acquired" );
        ...
        bool upgrade_to_writer() {
            x86_rtrw_mutex* mutex = x86_rtrw_mutex::internal_get_mutex(my_scoped_lock);
            __TBB_ASSERT( mutex, "lock is not acquired" );
            if (transaction_state == RTtransacting_writer || transaction_state == RTreal_writer)
        ...
        bool downgrade_to_reader() {
            x86_rtrw_mutex* mutex = x86_rtrw_mutex::internal_get_mutex(my_scoped_lock);
            __TBB_ASSERT( mutex, "lock is not acquired" );
            if (transaction_state == RTtransacting_reader || transaction_state == RTreal_reader)
        ...
        bool try_acquire( x86_rtrw_mutex& m, bool write = true ) {
#if TBB_USE_ASSERT
            x86_rtrw_mutex* mutex = x86_rtrw_mutex::internal_get_mutex(my_scoped_lock);
            ...
            if(transaction_state == RTtransacting_reader) return true;
            if( my_scoped_lock.try_acquire(m, false)) {
                transaction_state = RTreal_reader;
                return true;
            }
            return false;
        }

        };  // class x86_rtrw_mutex::scoped_lock
...
#endif /* __TBB__x86_rtrw_mutex_impl_H */
