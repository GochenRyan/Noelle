...
bgfx::TextureHandle RenderContext::CreateTexture(const char* pFilePath, uint64_t flags)
{
	...
	std::string textureFileFullPath = std::format("{}Textures/{}", CDEDITOR_RESOURCES_ROOT_PATH, pFilePath);
...
bgfx::VertexLayout RenderContext::CreateVertexLayout(StringCrc resourceCrc, const std::vector<VertexAttributeLayout>& vertexAttributes)
{
...
bgfx::VertexLayout RenderContext::CreateVertexLayout(StringCrc resourceCrc, const VertexAttributeLayout& vertexAttribute)
{
	auto itVertexLayoutCache = m_vertexLayoutCaches.find(resourceCrc.Value());
	if (itVertexLayoutCache != m_vertexLayoutCaches.end())
	{
